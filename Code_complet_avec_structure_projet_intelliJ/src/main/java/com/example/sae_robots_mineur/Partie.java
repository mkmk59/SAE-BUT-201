...
public class Partie {
    ...
    public void initialiserPartie(int nb_robot_Or,int nb_robot_Ni,int nb_Mines_Or, int nb_Mines_Ni) {
        ...
        while (nb_robot_Or_place <= nb_robot_Or) {
            int capacite_minerai_max = generateur.nextInt(5,10);
            int capacite_extraction = generateur.nextInt(1,4);
            ...
            int ok_coor=0;
            boolean ok_tab=false;
            if (liste_coordonnees_robot.size()>0){
                // Vérifier que les coordonnées générées n'ont pas déjà été utilisées
                while (ok_tab!=true) {
                    for (int[] tab : liste_coordonnees_robot) {
                        if (tab[0] != abs && tab[1] != ord) {
                            ok_coor=ok_coor+1;
                        }
                        else{
                            break;
                    ...
                    if (ok_coor==liste_coordonnees_robot.size()) {
        ...
        while (nb_robot_Ni_place <= nb_robot_Ni) {
            int capacite_minerai_max = generateur.nextInt(5,10);
            int capacite_extraction = generateur.nextInt(1,4);
            Robot robot_Ni = new Robot(nb_robot_Ni_place+nb_robot_Or, Robot.Specialite.Ni,capacite_minerai_max,capacite_extraction);
